    ...
    <Listbox value={currentPersona} onChange={setPersona}>
      <div className="relative mt-1">
        ...
        <Transition
          ...
          leaveTo="opacity-0"
        >
          <Listbox.Options className="absolute w-full py-1 mt-1 overflow-auto text-base bg-dark-deeper rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none z-10 glass">
            {personas.map((persona) => (
              <Listbox.Option
                key={persona.id}
                className={({ active }) =>
                  `${active ? 'bg-primary/20 text-white' : 'text-gray-light'}
